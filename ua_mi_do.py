...
This code is the code for Uncertainty-Aware Multiple Imputation (UA-MI) framework based on dropout-based method.
It consists of 3 parts: Training Multinomial Variational Autoencoder (MultVAE), performing UA-MI with dropout, re-training MultVAE with filled matrix.
...
def training_do(data, batch_size_train = 500, batch_size_valid = 2000, batch_size_test = 2000, total_anneal_steps = 200000, anneal_max = 0.2, n_epochs = 200):
    ...
    with tf.Session() as train_sess:
        ...
        for epoch in range(n_epochs):
            ...
            if ndcg_ > best_ndcg:
                saver.save(train_sess, '{}/trained_multvae'.format(chkpt_path))
                best_ndcg = ndcg_
...
# This function is for performing UA-MI based on dropout-based method.

def UA_MI_do(data, do_rate, fill_rate, n_ensembles):

    batch_size_train, batch_size_test, chkpt_path, dec_dim, idxlist_train, idxlist_test, N_train, N_test, n_items, train_data, test_data_fi, test_data_fo = training_do(data)

    tf.reset_default_graph()

    ua_mi_base = UA_MI_base(dec_dim, lam = 0.0, random_seed = 98765)
    saver, _, _, _, _ = ua_mi_base.build_graph()

    pred_rate = ua_mi_base.dropout()

    with tf.Session() as ua_mi_do_sess:
        saver.restore(ua_mi_do_sess, chkpt_path + '/trained_multvae')
        ...
        for bnum, st_idx in enumerate(range(0, N_train, batch_size_train)):
            ...
            for k in range(n_ensembles):
                pred_X = np.append(pred_X, ua_mi_do_sess.run(pred_rate, feed_dict = {ua_mi_base.input_interaction: X, ua_mi_base.keeping_prob_dropout: do_rate}))
...
def re_training_do(data, do_rate, fill_rate, n_ensembles, batch_size_train = 500, batch_size_valid = 2000, batch_size_test = 2000, total_anneal_steps = 200000, anneal_max = 0.2, n_epochs = 200):

    filled_X = UA_MI_do(data, do_rate, fill_rate, n_ensembles)
    ...
    with tf.Session() as re_train_sess:
        ...
        for epoch in range(n_epochs):
            ...
            if ndcg_ > best_ndcg:
                saver.save(re_train_sess, '{}/re_trained_do_{}_fill_{}_k_{}'.format(chkpt_path, do_rate, fill_rate, n_ensembles))
                best_ndcg = ndcg_
