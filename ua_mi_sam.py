...
This code is the code for Uncertainty-Aware Multiple Imputation (UA-MI) framework based on sampling-based method.
It consists of 3 parts: Training Multinomial Variational Autoencoder (MultVAE), performing UA-MI with sampling, re-training MultVAE with filled matrix.
...
def training_sam(data, batch_size_train = 500, batch_size_valid = 2000, batch_size_test = 2000, total_anneal_steps = 200000, anneal_max = 0.2, n_epochs = 200):
...
# This function is for performing UA-MI based on sampling-based method.

def UA_MI_sam(data, fill_rate, n_samples):

    batch_size_train, batch_size_test, chkpt_path, dec_dim, idxlist_train, idxlist_test, N_train, N_test, n_items, train_data, test_data_fi, test_data_fo = training_sam(data)
    ...
    pred_rate = ua_mi_base.sampling()

    with tf.Session() as ua_mi_sam_sess:
        saver.restore(ua_mi_sam_sess, chkpt_path + '/trained_multvae')
        ...
        for bnum, st_idx in enumerate(range(0, N_train, batch_size_train)):
            ...
            for k in range(n_samples):
                pred_X = np.append(pred_X, ua_mi_sam_sess.run(pred_rate, feed_dict = {ua_mi_base.input_interaction: X}))

            pred_X = pred_X.reshape(n_samples, X.shape[0], X.shape[1])
...
def re_training_sam(data, fill_rate, n_samples, batch_size_train = 500, batch_size_valid = 2000, batch_size_test = 2000, total_anneal_steps = 200000, anneal_max = 0.2, n_epochs = 200):

    filled_X = UA_MI_sam(data, fill_rate, n_samples)
    ...
    with tf.Session() as re_train_sess:
        ...
        for epoch in range(n_epochs):
            ...
            if ndcg_ > best_ndcg:
                saver.save(re_train_sess, '{}/re_trained_sam_fill_{}_k_{}'.format(chkpt_path, fill_rate, n_samples))
                best_ndcg = ndcg_
